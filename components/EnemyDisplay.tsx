...
import { Enemy, BodyPart } from '../types';
...
export const EnemyDisplay: React.FC<EnemyDisplayProps> = ({ enemy, onTargetPart }) => {
    return (
        <div className="flex flex-col sm:flex-row gap-4 p-4 border border-gray-700 bg-red-900/10 rounded-lg">
            <div className="w-full sm:w-1/3 flex flex-col items-center">
                <p className="text-xs text-yellow-300/80 italic mb-1 font-semibold">[ Click vào bộ phận để nhắm ]</p>
                <BodyStatus bodyParts={enemy.bodyParts} interactive={true} onPartClick={(part) => onTargetPart(enemy.id, part)} />
            </div>
