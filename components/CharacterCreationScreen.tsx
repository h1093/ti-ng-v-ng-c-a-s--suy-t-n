...
const BASE_STATS_TEMPLATE: CharacterStats = {
  hp: 50, maxHp: 50, san: 50, maxSan: 50, mana: 20, maxMana: 20,
  stamina: 50, maxStamina: 50, attack: 5, defense: 5, speed: 5, charisma: 5
};

const STATS_FOR_POINT_BUY: (keyof CharacterStats)[] = ['hp', 'san', 'mana', 'stamina', 'attack', 'defense', 'speed', 'charisma'];


export const CharacterCreationScreen: React.FC<CharacterCreationScreenProps> = ({ onFinish, mode }) => {
  ...
  const handlePointChange = (stat: keyof CharacterStats, amount: number) => {
  ...
  const finalStats = useMemo(() => {
    const final = { ...BASE_STATS_TEMPLATE };
    if (selectedOrigin) {
      for (const [key, value] of Object.entries(selectedOrigin.baseStats)) {
        final[key as keyof CharacterStats] = (BASE_STATS_TEMPLATE[key as keyof CharacterStats] || 0) + Number(value);
      }
    }
    for (const [key, value] of Object.entries(spentPoints)) {
        final[key as keyof CharacterStats] = (final[key as keyof CharacterStats] || 0) + (Number(value) || 0);
    }
  ...
  const remainingPoints = selectedDifficulty.pointBuy - Object.values(spentPoints).reduce((a, b) => a + (Number(b) || 0), 0);
  ...
  const handleSubmit = () => {
    ...
    const character: Character = {
      ...
      magicMasteries: ALL_MAGIC_SCHOOLS.reduce((acc, school) => {
        acc[school] = { level: 1, xp: 0, xpToNextLevel: 100, unlocked: true };
        return acc;
      ...
      faith: ALL_DEITIES.reduce((acc, deity) => {
  ...
  const renderSection = (title: string, children: React.ReactNode, extraClasses = "") => (
      ...
      {mode === 'custom' && renderSection("Kịch Bản Tùy Chỉnh", (
        <div>
          ...
          <textarea value={customScenario} onChange={e => setCustomScenario(e.target.value)}
            ...
            rows={5} className="w-full bg-gray-900 border border-gray-700 p-2 rounded focus:outline-none focus:ring-2 focus:ring-purple-500"
          />
        </div>
      ), "border-purple-500")}
      ...
      {renderSection("Thông Tin Cơ Bản", (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            <textarea value={backstory} onChange={e => setBackstory(e.target.value)} placeholder="Tiểu sử (tùy chọn, hoặc để AI tạo)" rows={4} className="w-full bg-gray-900 border border-gray-700 p-2 rounded focus:outline-none focus:ring-2 focus:ring-red-500" />
