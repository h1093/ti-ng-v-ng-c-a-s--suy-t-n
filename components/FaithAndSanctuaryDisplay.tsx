...
import type { Character, DeityName } from '../types';

export const deityColors: Record<DeityName, { text: string; bg: string; border: string }> = {
...
const FaithAndSanctuaryDisplay: React.FC<{character: Character}> = ({ character }) => {
        ...
        <div className="p-4 space-y-6">
            <div>
                ...
                <div className="space-y-4">
                    {(Object.keys(faith) as DeityName[]).map(deityName => {
                        const status = faith[deityName];
                        if (!status) return null; // Add a guard for safety
                ...
                <div className="pt-6 border-t border-gray-700">
                    <h3 className="font-bold text-gray-300 text-xl mb-4 text-center">Thánh Địa: {sanctuary.name}</h3>
                     <div className="grid grid-cols-2 gap-4 text-center bg-black/20 p-3 rounded-md border border-gray-700/50">
                        <div><span className="text-gray-400 uppercase text-xs">Hy Vọng</span><p className="font-bold text-2xl text-white">{sanctuary.hope}</p></div>
                        <div><span className="text-gray-400 uppercase text-xs">Dân Số</span><p className="font-bold text-2xl text-white">{sanctuary.population}</p></div>
                     </div>
...
export default FaithAndSanctuaryDisplay;